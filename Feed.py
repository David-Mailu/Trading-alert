...
def get_xauusd_4hr_candles(max_retries=3, delay_seconds=3):
    ...
    print("❌ All retries failed — no valid candle retrieved.")
    return None
def get_xauusd_1hr_candles(max_retries=3, delay_seconds=3):
    symbol = "XAUUSD"
    timeframe = mt5.TIMEFRAME_H1  # ⏱️ 1-hour timeframe
    num_candles = 4  # Fetch last 4 candles including the current forming one
    ...
    for attempt in range(1, max_retries + 1):
        ...
        try:
            ...
            # Fetch 4 candles starting from the current forming one
            candles = mt5.copy_rates_from_pos(symbol, timeframe, 0, num_candles)
            ...
            for i, candle in enumerate(candles):
                ...
                print(f"🕯️ 1hr Candle {i+1} ➝ Open: {open_}, Close: {close}, Size: {size:.2f}, Volume: {volume}, Time: {timestamp}")
